			...
			extension Bindings {
				...
				internal class Vec_TransactionOutputsZ: NativeTypeWrapper {
					...
					public init(cType: LDKCVec_TransactionOutputsZ) {
					...
					public init(cType: LDKCVec_TransactionOutputsZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(array: [([UInt8], [(UInt32, TxOut)])]) {
						...
						let currentValueDepth1Tuple = Tuple_TxidCVec_C2Tuple_u32TxOutZZZ(tuple: currentValueDepth1)
...
Tuple_TxidCVec_C2Tuple_u32TxOutZZZ(cType: currentCType).getValue()
						}
						return swiftArray
					}
					...
					internal func dangle() -> Vec_TransactionOutputsZ {
        				self.dangling = true
						return self
					}
