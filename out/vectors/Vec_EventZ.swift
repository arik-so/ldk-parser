			...
			extension Bindings {
				...
				internal class Vec_EventZ: NativeTypeWrapper {
					...
					public init(cType: LDKCVec_EventZ) {
					...
					public init(cType: LDKCVec_EventZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public func getValue() -> [Event] {
						...
						let swiftArray = array.map { (currentCType: LDKEvent) in
Event(cType: currentCType, anchor: self).dangle()
					...
					internal func dangle() -> Vec_EventZ {
        				self.dangling = true
						return self
					}
