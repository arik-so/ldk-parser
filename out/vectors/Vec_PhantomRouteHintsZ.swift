			...
			extension Bindings {
				...
				internal class Vec_PhantomRouteHintsZ: NativeTypeWrapper {
					...
					public init(cType: LDKCVec_PhantomRouteHintsZ) {
					...
					public init(cType: LDKCVec_PhantomRouteHintsZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(array: [PhantomRouteHints]) {
						...
						let rustArray = array.map { (currentValueDepth1: PhantomRouteHints) in
					...
					public func getValue() -> [PhantomRouteHints] {
						...
						let swiftArray = array.map { (currentCType: LDKPhantomRouteHints) in
