			...
			extension Bindings {
				...
				internal class Vec_UpdateFulfillHTLCZ: NativeTypeWrapper {
					...
					public init(cType: LDKCVec_UpdateFulfillHTLCZ) {
					...
					public init(cType: LDKCVec_UpdateFulfillHTLCZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(array: [UpdateFulfillHTLC]) {
						...
						let rustArray = array.map { (currentValueDepth1: UpdateFulfillHTLC) in
					...
					public func getValue() -> [UpdateFulfillHTLC] {
						...
						let swiftArray = array.map { (currentCType: LDKUpdateFulfillHTLC) in
