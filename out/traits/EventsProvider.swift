				...
				open class EventsProvider: NativeTraitWrapper {
					...
					public init() {
						...
						func processPendingEventsLambda(this_arg: UnsafeRawPointer?, handler: LDKEventHandler) -> Void {
							...
							// Swift callback call
							let swiftCallbackResult = instance.processPendingEvents(handler: NativelyImplementedEventHandler(pointer: handler))
					...
					internal func free() {
						...
						// native method call
						let nativeCallResult = EventsProvider_free(self.cType!)
