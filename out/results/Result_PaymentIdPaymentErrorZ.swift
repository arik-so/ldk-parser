			...
			extension Bindings {
				...
				public class Result_PaymentIdPaymentErrorZ: NativeTypeWrapper {
					...
					public init(o: [UInt8]) {
						...
						// native method call
						let nativeCallResult = CResult_PaymentIdPaymentErrorZ_ok(o.cType!)
					...
					public init(e: PaymentError) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = CResult_PaymentIdPaymentErrorZ_err(e.cType!)
			...
			CResult_PaymentIdPaymentErrorZ_clone(origPointer)
					...
					public func getError() -> PaymentError? {
						if self.cType?.result_ok == false {
							return PaymentError(pointer: self.cType!.contents.err.pointee)
						}
						return nil
					}
