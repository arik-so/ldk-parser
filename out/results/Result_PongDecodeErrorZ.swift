			...
			extension Bindings {
				...
				public class Result_PongDecodeErrorZ: NativeTypeWrapper {
					...
					public init(o: Pong) {
						...
						// native method call
						let nativeCallResult = CResult_PongDecodeErrorZ_ok(o.cType!)

						// cleanup
						

						// return value (do some wrapping)
						let returnValue = Result_PongDecodeErrorZ(pointer: nativeCallResult)
					...
					public init(e: DecodeError) {
						...
						// native method call
						let nativeCallResult = CResult_PongDecodeErrorZ_err(e.cType!)
			...
			CResult_PongDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> Pong? {
						if self.cType?.result_ok == true {
							return Pong(pointer: self.cType!.contents.result.pointee)
						}
						return nil
					}
