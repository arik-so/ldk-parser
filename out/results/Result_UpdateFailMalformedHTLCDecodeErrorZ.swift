			...
			extension Bindings {
				...
				public class Result_UpdateFailMalformedHTLCDecodeErrorZ: NativeTypeWrapper {
					...
					public init(o: UpdateFailMalformedHTLC) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = CResult_UpdateFailMalformedHTLCDecodeErrorZ_ok(o.cType!)

						// cleanup
						

						// return value (do some wrapping)
						let returnValue = Result_UpdateFailMalformedHTLCDecodeErrorZ(pointer: nativeCallResult)
					...
					public init(e: DecodeError) {
						...
						// native method call
						let nativeCallResult = CResult_UpdateFailMalformedHTLCDecodeErrorZ_err(e.cType!)
			...
			CResult_UpdateFailMalformedHTLCDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> UpdateFailMalformedHTLC? {
						if self.cType?.result_ok == true {
							return UpdateFailMalformedHTLC(pointer: self.cType!.contents.result.pointee)
						}
						return nil
					}
