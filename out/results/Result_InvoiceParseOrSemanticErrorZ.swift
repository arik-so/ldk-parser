			...
			extension Bindings {
				...
				public class Result_InvoiceParseOrSemanticErrorZ: NativeTypeWrapper {
					...
					public init(o: Invoice) {
						...
						// native method call
						let nativeCallResult = CResult_InvoiceParseOrSemanticErrorZ_ok(o.cType!)
					...
					public init(e: ParseOrSemanticError) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = CResult_InvoiceParseOrSemanticErrorZ_err(e.cType!)
			...
			CResult_InvoiceParseOrSemanticErrorZ_clone(origPointer)
					...
					public func getError() -> ParseOrSemanticError? {
						if self.cType?.result_ok == false {
							return ParseOrSemanticError(pointer: self.cType!.contents.err.pointee)
						}
						return nil
					}
					...
					public func getValue() -> Invoice? {
						if self.cType?.result_ok == true {
							return Invoice(pointer: self.cType!.contents.result.pointee)
						}
						return nil
					}
