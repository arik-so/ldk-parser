			...
			extension Bindings {
				...
				public class Result_NodeIdDecodeErrorZ: NativeTypeWrapper {
					...
					public init(cType: LDKCResult_NodeIdDecodeErrorZ) {
					...
					public init(cType: LDKCResult_NodeIdDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(o: NodeId) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = CResult_NodeIdDecodeErrorZ_ok(o.cType!)
				...
				super.init(conflictAvoidingVariableName: 0)
					...
					public init(e: DecodeError) {
						...
						// native method call
						let nativeCallResult = CResult_NodeIdDecodeErrorZ_err(e.cType!)
				...
				CResult_NodeIdDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> NodeId? {
						if self.cType?.result_ok == true {
							return NodeId(cType: self.cType!.contents.result.pointee, anchor: self).dangle()
						}
						return nil
					}
