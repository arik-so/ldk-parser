			...
			extension Bindings {
				...
				public class Result_ChannelInfoDecodeErrorZ: NativeTypeWrapper {
					...
					public init(cType: LDKCResult_ChannelInfoDecodeErrorZ) {
					...
					public init(cType: LDKCResult_ChannelInfoDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(o: ChannelInfo) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = CResult_ChannelInfoDecodeErrorZ_ok(o.cType!)
				...
				super.init(conflictAvoidingVariableName: 0)
					...
					public init(e: DecodeError) {
						...
						// native method call
						let nativeCallResult = CResult_ChannelInfoDecodeErrorZ_err(e.cType!)
				...
				CResult_ChannelInfoDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> ChannelInfo? {
						if self.cType?.result_ok == true {
							return ChannelInfo(cType: self.cType!.contents.result.pointee, anchor: self).dangle()
						}
						return nil
					}
