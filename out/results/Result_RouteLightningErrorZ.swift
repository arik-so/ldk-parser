			...
			extension Bindings {
				...
				public class Result_RouteLightningErrorZ: NativeTypeWrapper {
					...
					public init(o: Route) {
						...
						// native method call
						let nativeCallResult = CResult_RouteLightningErrorZ_ok(o.cType!)
					...
					public init(e: LightningError) {
						...
						// native method call
						let nativeCallResult = CResult_RouteLightningErrorZ_err(e.cType!)
			...
			CResult_RouteLightningErrorZ_clone(origPointer)
					...
					public func getError() -> LightningError? {
						if self.cType?.result_ok == false {
							return LightningError(pointer: self.cType!.contents.err.pointee)
						}
						return nil
					}
					...
					public func getValue() -> Route? {
						if self.cType?.result_ok == true {
							return Route(pointer: self.cType!.contents.result.pointee)
						}
						return nil
					}
