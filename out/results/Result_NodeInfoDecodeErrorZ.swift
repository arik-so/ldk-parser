			...
			extension Bindings {
				...
				public class Result_NodeInfoDecodeErrorZ: NativeTypeWrapper {
					...
					public init(cType: LDKCResult_NodeInfoDecodeErrorZ) {
					...
					public init(cType: LDKCResult_NodeInfoDecodeErrorZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public class func initWithOk(o: NodeInfo) -> Result_NodeInfoDecodeErrorZ {
						...
						// native method call
						let nativeCallResult = CResult_NodeInfoDecodeErrorZ_ok(o.clone().cType!)
					...
					public class func initWithErr(e: DecodeError) -> Result_NodeInfoDecodeErrorZ {
						...
						// native method call
						let nativeCallResult = CResult_NodeInfoDecodeErrorZ_err(e.clone().cType!)
				...
				CResult_NodeInfoDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> NodeInfo? {
						if self.cType?.result_ok == true {
							return NodeInfo(cType: self.cType!.contents.result.pointee, anchor: self).dangle()
						}
						return nil
					}
