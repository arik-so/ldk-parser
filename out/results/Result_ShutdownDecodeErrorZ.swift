			...
			extension Bindings {
				...
				public class Result_ShutdownDecodeErrorZ: NativeTypeWrapper {
					...
					public init(o: Shutdown) {
						// native call variable prep
						

						// native method call
						let nativeCallResult = CResult_ShutdownDecodeErrorZ_ok(o.cType!)

						// cleanup
						

						// return value (do some wrapping)
						let returnValue = Result_ShutdownDecodeErrorZ(pointer: nativeCallResult)
					...
					public init(e: DecodeError) {
						...
						// native method call
						let nativeCallResult = CResult_ShutdownDecodeErrorZ_err(e.cType!)
			...
			CResult_ShutdownDecodeErrorZ_clone(origPointer)
					...
					public func getValue() -> Shutdown? {
						if self.cType?.result_ok == true {
							return Shutdown(pointer: self.cType!.contents.result.pointee)
						}
						return nil
					}
