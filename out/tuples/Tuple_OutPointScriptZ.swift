			...
			extension Bindings {
				...
				internal class Tuple_OutPointScriptZ: NativeTypeWrapper {
					...
					public init(cType: LDKC2Tuple_OutPointScriptZ) {
					...
					public init(cType: LDKC2Tuple_OutPointScriptZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
				...
				C2Tuple_OutPointScriptZ_clone(origPointer)
					...
					public init(a: OutPoint, b: [UInt8]) {
						...
						// native method call
						let nativeCallResult = C2Tuple_OutPointScriptZ_new(a.clone().setCFreeability(freeable: false).cType!, bVector.cType!)
