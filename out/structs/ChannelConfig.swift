			...
			extension Bindings {
				...
				public class ChannelConfig: NativeTypeWrapper {
					...
					public init(cType: LDKChannelConfig) {
					...
					public init(cType: LDKChannelConfig, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
			...
			ChannelConfig_clone(origPointer)
					...
					public func eq() -> Bool {
						...
						// native method call
						let nativeCallResult = 
						withUnsafePointer(to: self.cType!) { (aPointer: UnsafePointer<LDKChannelConfig>) in
			
						withUnsafePointer(to: self.cType!) { (bPointer: UnsafePointer<LDKChannelConfig>) in
			...
			ChannelConfig_write(objPointer)
					...
					public class func read(ser: [UInt8]) -> Result_ChannelConfigDecodeErrorZ {
						...
						// native method call
						let nativeCallResult = ChannelConfig_read(ser.cType!)
