			...
			extension Bindings {
				...
				public class ChannelManager: NativeTypeWrapper {
					...
					public init(feeEst: FeeEstimator, chainMonitor: Watch, txBroadcaster: BroadcasterInterface, logger: Logger, keysManager: KeysInterface, config: UserConfig, params: ChainParameters) {
						...
						// return value (do some wrapping)
						let returnValue = ChannelManager(pointer: nativeCallResult)
			...
			ChannelManager_send_spontaneous_payment(thisArgPointer, routePointer, paymentPreimage.cType!)
						...
						// return value (do some wrapping)
						let returnValue = Result_C2Tuple_PaymentHashPaymentIdZPaymentSendFailureZ(pointer: nativeCallResult)
			...
			ChannelManager_get_persistable_update_future(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = Future(pointer: nativeCallResult)
			...
			ChannelManager_as_Payer(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedPayer(pointer: nativeCallResult, anchor: self)
