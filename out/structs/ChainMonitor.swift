			...
			extension Bindings {
				...
				ChainMonitor_get_monitor(thisArgPointer, fundingTxo.dynamicallyDangledClone().cType!)
						...
						// return value (do some wrapping)
						let returnValue = Result_LockedChannelMonitorNoneZ(cType: nativeCallResult, anchor: self).dangle(false)
				...
				ChainMonitor_as_Watch(thisArgPointer)
						...
						// return value (do some wrapping)
						let returnValue = NativelyImplementedWatch(cType: nativeCallResult, anchor: self).dangle(false)
