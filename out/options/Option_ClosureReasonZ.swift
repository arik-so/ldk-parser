			...
			extension Bindings {
				...
				internal class Option_ClosureReasonZ: NativeTypeWrapper {
					...
					public init(cType: LDKCOption_ClosureReasonZ) {
					...
					public init(cType: LDKCOption_ClosureReasonZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(some: ClosureReason?) {
						...
						if let some = some {
							...
							self.cType = COption_ClosureReasonZ_some(some.clone().cType!)
						} else {
