			...
			extension Bindings {
				...
				internal class Option_MonitorEventZ: NativeTypeWrapper {
					...
					public init(cType: LDKCOption_MonitorEventZ) {
					...
					public init(cType: LDKCOption_MonitorEventZ, anchor: NativeTypeWrapper) {
						Self.instanceCounter += 1
						self.instanceNumber = Self.instanceCounter
						self.cType = cType
						super.init(conflictAvoidingVariableName: 0)
					...
					public init(some: MonitorEvent?) {
						...
						if let some = some {
							...
							self.cType = COption_MonitorEventZ_some(some.clone().cType!)
						} else {
